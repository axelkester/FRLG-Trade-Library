...
5. On the Switch: Direct Corner → trade room → **Leader**.
6. Press OK on the web app popup message.
7. Accept the join from the simulated trainer (default name "EMU"), walk to the left chair, select a
   Pokémon, and accept the trade confirmation (same steps as the CLI README).
8. Watch the state machine in the side panel: SCANNING → JOINING → CONNECTED →
   TRADING → COMPLETED, with the live `frlgtrade.py` log streamed underneath.
9. The received Pokémon lands in `received/` with a collision-safe name; its
